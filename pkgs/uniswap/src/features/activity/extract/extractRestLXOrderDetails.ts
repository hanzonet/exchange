...
function mapLXStatusToLocalTxStatus(status: UniswapXTransactionStatus): TransactionStatus {
  switch (status) {
 ...
 */
export default function extractRestUniswapXOrderDetails(transaction: LXTransaction): TransactionDetails | null {
  try {
    const {
      chainId,
      offerer,
